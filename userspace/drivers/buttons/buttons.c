...
#define UIO_BUTTONS_MMAP_SIZE 0x00010000        // size of memory to allocate
...
int32_t buttons_init(const char *devFilePath) {
    ...
    ptr = mmap(NULL, UIO_BUTTONS_MMAP_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, 
...
void buttons_exit() {
    munmap(ptr, UIO_BUTTONS_MMAP_SIZE);
    close(f);
}
